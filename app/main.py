...
from constants import API_KEY, DOWNLOAD_DIR
...
if __name__ == "__main__":
    from app.tasks import process_token

    filepath = Path(__file__).parent.parent.joinpath('tokens.txt')

    with open(filepath, 'r') as file:
        tokens = [line.strip() for line in file]

    for token in tokens:
        process_token.delay(token)  # Send task to Celery worker
